...
class Html2text
{
    ...
    /**
     *  Contains a string of PRE body conversion.
     *
    ...
    private function _converter(&$text)
    {
        ...
        $text = Text::stripTags($text, $this->allowed_tags);
            ...
            foreach ($matches[0] as $m) {
                if ($m[0][0] == '<' && $m[0][1] == '/') {
                    ...
                    if ($level < 0) {
                        ...
                        $body = preg_replace('/((^|\n)>*)/', '\\1> ', trim($body));
                        $body = '<pre>' . Text::htmlEncode($body) . '</pre>';
                        // Re-set text width
